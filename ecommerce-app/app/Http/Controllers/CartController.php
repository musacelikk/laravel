...
class CartController extends Controller
{
    ...
    public function addFromLink(Request $request, Product $product): RedirectResponse
    {
        if (! $product->inStock()) {
            return redirect()
                ->route('shop.index')
                ->with('error', $product->name.' is currently out of stock.');
        }

        $cart = $request->session()->get('cart', []);
        $cart[$product->id] = min(99, ($cart[$product->id] ?? 0) + 1);
        $request->session()->put('cart', $cart);

        return redirect()
            ->route('cart.index')
            ->with('success', $product->name.' added to your bag.');
    }
